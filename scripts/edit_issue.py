#!/usr/bin/env python3
"""每日编辑:读存档 + 候选素材,分两段调 Kimi Code API 产出当期内容(JSON)。

- 调用 A(选稿写作):候选素材 + 已收录 URL(归一化)→ 当期 items
- 调用 B(选题策划):定稿 items + 历期选题 + 存档近 60 天条目 → ideas
  两段都走 call_kimi(SSE 流式),合并后统一过 validate 校验契约。
...
- --mock 模式:不调 API,产出与真实路径同结构的假 issue(4 学术+2 开源+3 行业,3 选题)
...
from fetch_sources import normalize_url
...
# ── 调用 A:选稿写作 ─────────────────────────────────────────────
PROMPT_ITEMS_TMPL = """你是「音乐科技雷达」的每日编辑。这是一份面向浙江音乐学院『数字音乐智能处理』专业学生的每日情报报纸,覆盖音乐×计算机(AI 音乐生成、MIR、音频处理)与音乐×视觉/交互艺术的学术与行业动态。你的任务:从下面的候选素材中筛选最近 24–48 小时的最新内容,产出当期报纸条目(JSON)。

【候选来源说明】
候选素材来自:arXiv、OpenAlex、Hugging Face 每日论文(学术);GitHub、Hugging Face 模型(开源);Hacker News、Google News 中英文 RSS、行业媒体 RSS(cdm.link / musictech.com / synthtopia)、Bluesky 社交帖(行业)。注意:行业 RSS、社交帖、模型平台这些源噪声很大(广告、软文、闲聊、重复转载居多),筛选标准不变,但更要「宁缺毋滥」:拿不准的一律不收,社交帖只收有实质信息量的(新工具发布、一手行业消息)。

【筛选标准】
- 只保留与专业强相关的:音乐/歌声生成、符号音乐、MIR(转谱/检索/推荐/评测)、音频信号处理与音效、音乐×视觉/VR/交互艺术、新型音乐交互界面(NIME 类)、音乐科技行业新闻(版权诉讼、产品发布、公司动态)。
- 排除:纯语音技术(ASR/TTS/说话人识别,除非与音乐直接相关)、生物声学、与音乐无关的音频工程、GitHub 上的盗版软件和 SEO 垃圾仓库、社交平台的情绪吐槽与广告。
- 去重:下列「已收录 URL」(已归一化)任何一条都不得再次出现在你的输出中;同一内容多个来源只取一条。
- 数量:学术(papers)5–8 篇、开源(oss)2–4 个、行业(industry)3–5 条。宁缺毋滥,某板块没有好内容就少放。
- 恰好 2 条头条:学术板块选 1 条标 hot="本报头条",行业板块选 1 条标 hot="行业要闻",其余条目不带 hot 字段。

【写作要求】
- 全部用中文,文字凝练。
- 每条 items 字段:sec(papers|oss|industry)、tag(短分类,如 音乐生成/符号音乐/评测/版权)、date(条目自身日期 YYYY-MM-DD)、title(中文标题)、url、src(来源短标注,如 "arXiv · cs.SD"、"HN 396pt"、"C++ · 842★"、"Bluesky @handle")、orig(论文英文原题,非论文省略)、why(凝练一两句:做了什么+为什么值得看;头条条目写两句——第一句『做了什么』,第二句『为什么值得看』,两句都以句号结尾)、detail(详情正文)。
...
# ── 调用 B:选题策划 ─────────────────────────────────────────────
PROMPT_IDEAS_TMPL = """你是「音乐科技雷达」的选题策划。这份报纸面向浙江音乐学院『数字音乐智能处理』专业学生(会乐理、编程能力中等)。本期条目已定稿(见下),你的任务:结合历期积累,产出 3–4 个学生可动手的作品/研究选题(JSON)。

【质量要求】
- 每个选题必须写清两件事:做什么(具体的产出物,不是方向)、第一步怎么动手(今天就能执行的动作)。说不清这两条的一律不要。
- 每个选题必须用 based_on 引用具体条目作为依据:本期条目直接引用;引用往期条目时额外带 vol 字段标注期号。不允许没有依据的空想选题。
- 禁止空泛选题,例如「做一个了解 AI 音乐的工具」这种没有具体产出物、没有动手指引的一律不合格。
- 基于存档中近 60 天的全部积累(不只是本期条目!),鼓励跨期组合;好选题可以连续保留多期(沿用下方「历期选题」中的选题,基于的条目标注首提期号),但至少 1 个全新选题或明显升级的旧选题。
- 按难度从低到高排列,跨度从课程作业到可发论文。

【输出契约】只输出一个 JSON 对象,不要输出任何其他文字。schema:
{"ideas":[{"level":"入门|进阶|硬核","kind":"工具|交互作品|产品原型|研究","title":"..","body":"做什么+第一步怎么动手,结尾用『需要:』列出所需技能","based_on":[{"vol":可选,"url":"..","title":".."}]}]}
...
【存档近 60 天条目(vol/tag/title/url)】
{recent_items}
"""

# ── --mock 假数据:与真实路径同结构(4 学术 + 2 开源 + 3 行业,3 选题)──
MOCK_ITEMS = [
    ...
    {"sec": "industry", "tag": "硬件", "date": "2026-07-24",
     "title": "示例:某合成器厂商推出带 AI 音色设计的桌面合成器",
     "url": "https://example.com/news/ai-synth-hardware", "src": "示例行业媒体",
     "why": "用自然语言描述生成合成器音色的硬件产品首次量产。AI 音色设计从软件走向硬件工作台。",
     "detail": "【这是什么】一家合成器厂商发布了一款桌面硬件合成器:在屏幕上输入『像下雨天的电钢琴』这样的描述,内置模型直接生成对应的音色参数,可以继续在面板上手动调整。\n【为什么值得关注】AI 音色设计此前只存在于插件和论文里,这是第一次做成量产硬件。它展示了『自然语言 → 合成参数』这条技术路线的商业化成熟度。\n【怎么入手】看官方演示视频,留意它生成音色后还保留了哪些手动控制——『AI 生成 + 手动精修』的配比设计值得借鉴到自己的作品里。"},
]

MOCK_IDEAS = [
    {"level": "入门", "kind": "工具", "title": "「和弦速查」练耳小工具",
     "body": "用 chordscan 给曲库批量标注和弦,做一个按和弦进行检索歌曲的练习工具。第一步:pip 安装 chordscan,拿 10 首熟悉的歌跑一遍,人工核对标注准确率。需要:Python 基础 · 命令行使用。",
     "based_on": [{"url": "https://github.com/example/chordscan", "title": "chordscan"}]},
    {"level": "进阶", "kind": "交互作品", "title": "网页效果器声音装置",
     "body": "基于 pedalboard-lite 的效果器链,做一个『观众动作改变效果参数』的网页声音装置。第一步:fork 项目,把延迟效果器的反馈参数绑到鼠标位置上跑通最小原型。需要:JavaScript · Web Audio 基础。",
    ...
    {"level": "硬核", "kind": "研究", "title": "中文歌 AI 伴奏听测研究",
     "body": "沿用大规模听测方法,构建中文流行歌 AI 伴奏盲听实验,对比开源与商业系统的差距。第一步:复现原文的问卷结构,用 5 首中文歌做小规模预实验。需要:实验设计 · 统计分析。",
     "based_on": [{"url": "https://arxiv.org/abs/2607.90003", "title": "AI 伴奏听测研究"},
                  {"vol": 1, "url": "https://arxiv.org/abs/2607.19688", "title": "AI 翻唱诊断评测"}]},
]


def build_items_prompt(archive, raw):
    """调用 A 的 prompt:候选素材 + 归一化后的已收录 URL。"""
    items = archive.get("items", [])
    known_urls = "\n".join(sorted({normalize_url(it.get("url", ""))
                                   for it in items if it.get("url")})) or "(空)"
    ...
    return (PROMPT_ITEMS_TMPL.replace("{known_urls}", known_urls)
                             .replace("{candidates}", candidates))


def build_ideas_prompt(archive, items):
    """调用 B 的 prompt:定稿 items(完整含 detail)+ 历期选题 + 存档近 60 天条目清单。"""
    ...
    cutoff = (datetime.now(TZ).date() - timedelta(days=60)).isoformat()
    recent = json.dumps(
        [{"vol": it.get("vol"), "tag": it.get("tag"),
          "title": it.get("title"), "url": it.get("url")}
         for it in archive.get("items", []) if (it.get("pub") or "") >= cutoff],
        ensure_ascii=False, indent=1) or "(空)"
    items_json = json.dumps(items, ensure_ascii=False, indent=1)
    return (PROMPT_IDEAS_TMPL.replace("{items}", items_json)
                             .replace("{known_ideas}", known_ideas)
                             .replace("{recent_items}", recent))
...
def _parse_json(content, label):
    """解析 API 输出为 JSON,失败打印原始输出并退出。"""
    try:
        return json.loads(content)
    except json.JSONDecodeError:
        print(f"[edit] 错误:{label} 输出不是合法 JSON,原始输出如下:", file=sys.stderr)
...
def validate(issue, archive):
    ...
    URL 比对前双方都做归一化(与 fetch_sources 入库口径一致)。
    """
    errs = []
    ...
    known_urls = {normalize_url(it.get("url", ""))
                  for it in archive.get("items", []) if it.get("url")}
    seen = set()
...
def main():
    ...
    if args.mock:
        issue = {"items": [dict(it) for it in MOCK_ITEMS],
                 "ideas": [dict(idea) for idea in MOCK_IDEAS]}
        ...
        # 调用 A:选稿写作 → items
        prompt_a = build_items_prompt(archive, raw)
        print(f"[edit] 调用 A(选稿写作)prompt {len(prompt_a)} 字符,调用 {MODEL} ...")
        data_a = _parse_json(call_kimi(prompt_a, api_key), "调用 A")
        items = data_a.get("items")
        if not isinstance(items, list) or not items:
            print("[edit] 错误:调用 A 输出缺少 items 数组", file=sys.stderr)
            sys.exit(1)
        print(f"[edit] 调用 A 完成:{len(items)} 条 items")

        # 调用 B:选题策划(输入定稿 items)→ ideas
        prompt_b = build_ideas_prompt(archive, items)
        print(f"[edit] 调用 B(选题策划)prompt {len(prompt_b)} 字符,调用 {MODEL} ...")
        data_b = _parse_json(call_kimi(prompt_b, api_key), "调用 B")
        ideas = data_b.get("ideas")
        if not isinstance(ideas, list) or not ideas:
            print("[edit] 错误:调用 B 输出缺少 ideas 数组", file=sys.stderr)
            sys.exit(1)
        print(f"[edit] 调用 B 完成:{len(ideas)} 个选题")

        issue = {"items": items, "ideas": ideas}
